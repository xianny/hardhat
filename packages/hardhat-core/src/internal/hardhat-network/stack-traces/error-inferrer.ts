...
export interface SubmessageData {
  message: MessageTrace; // fvtodo rename to messageTrace
  trace: SolidityStackTrace; // fvtodo rename to stacktrace
  stepIndex: number;
}
...
export class ErrorInferrer {
  ...
  public inferAfterTracing(
    ...
    if (lastSubmessageData !== undefined) {
      const lastMessageFailed = lastSubmessageData.message.error !== undefined;

      if (lastMessageFailed) {
        if (
          this._isSubtraceErrorPropagated(
            trace,
          ...
          stacktrace.push(...lastSubmessageData.trace);
          ...
          return this._fixInitialModifier(trace, stacktrace);
        }
        ...
        if (this._isReturnDataSizeError(trace, lastSubmessageData.stepIndex)) {
          stacktrace.push({
    ...
    return (
      ...
      this._checkRevertOrInvalidOpcode(
        trace,
        stacktrace,
        jumpedIntoFunction,
        functionJumpdests
      ) ??
      this._checkLastInstruction(trace, stacktrace, jumpedIntoFunction) ??
  ...
  private _checkRevertOrInvalidOpcode(
    ...
    const lastRevertOrInvalid = this._getLastRevertOrInvalid(trace);
    if (lastRevertOrInvalid !== undefined) {
      const inst = lastRevertOrInvalid;
      if (
        ...
        if (failingFunction !== undefined) {
          stacktrace.push(
            this._instructionWithinFunctionToRevertStackTraceEntry(trace, inst)
          );
  ...
  private _getLastRevertOrInvalid(
    trace: DecodedEvmMessageTrace
  ): Instruction | undefined {
    for (let stepIndex = trace.steps.length - 1; stepIndex >= 0; stepIndex--) {
      const step = trace.steps[stepIndex];
      ...
      if (inst.opcode === Opcode.REVERT || inst.opcode === Opcode.INVALID) {
        return inst;
      }
    }
  }
